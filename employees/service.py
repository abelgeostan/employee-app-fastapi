...
async def create(db:AsyncSession, body: EmployeeCreate)->Employee:
    ...
    employee=Employee(name=body.name,email=body.email,password_hash=hashed,age=body.age, role=body.role)
    if body.address:
...
async def delete(db:AsyncSession,id:int):
    db_employee= await repo.get_by_id(db,id)
    if not db_employee:
        raise NotFoundException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Employee {id} not found")
...
async def delete_address(db: AsyncSession, emp_id: int, addr_id: int):
    ...
    if address.employee_id!=emp_id:
        raise ConflictException(detail=f"Address id {addr_id} dont match with employee {emp_id}")
