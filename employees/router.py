...
from employees.schemas import AddressResponse, EmployeeByIdResponse, EmployeeCreate, EmployeeResponse
...
async def get_by_id(id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
    employee=await service.get_by_id(db,id)
    return employee

@router.post("/{emp_id}/departments/{dept_id}")
async def attach_department_to_employee(emp_id: int, dept_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
    employee=await service.attach_department(db,emp_id,dept_id)
    return employee

@router.delete("/{emp_id}/departments/{dept_id}")
async def detach_department_from_employee(emp_id: int, dept_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
...
async def delete_address(emp_id: int, addr_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)):
